...
* [ ] Real mobile viewport smoke at 375x812, using actual browser/Playwright viewport, not CSS simulation.
  ...
  **Phase 3 status (branch `release/mobile-smoke-fixes`):** Real 375×812 + 1280 browser smoke executed (Claude Preview real Chromium viewport, NOT CSS simulation). Two release-blocking defects found and FIXED (frontend-only):
  ...
  All other listed surfaces (dashboard cap states <70/amber/rose/overspent-clamped, ActivityFeed, ממתין pill, /insights toggle + DB-color top_member avatar, /family/wishlists child lists + mark-fulfilled, /settings/category-budgets editor, limited dashboard + WishlistPanel, empty-household states) passed the 375 smoke. A final re-smoke on the merged release state is still recommended before deploy.
...
* [ ] limited_member privacy release smoke.
  ...
  **Phase 3 `/members` privacy decision (OPEN — awaiting approval):** Smoke confirmed `GET /households/:id/members` returns 200 to `limited_member` including every member's `phoneE164`. Audit found a real limited_member production dependency: **`/shopping-list`** (limited-accessible) calls `listMembers` unconditionally to render who-added avatars (confirmed at runtime — limited's /shopping-list issues the `/members` fetch). Per protocol a blanket 403 was therefore NOT applied (would emit 403 noise + drop who-added avatars for limited), and self-only was rejected (would break cross-member who-added on the shared list). **Recommended narrowest safe fix:** redact `phoneE164` (and any non-UI PII) from the `/members` response for `limited_member`, keeping `{userId, displayName, role, color, status}`; owner/admin/adult unchanged. This is a BACKEND change and is **NOT yet implemented** — all-member phone exposure to limited_member persists. **Release privacy blocker until the redaction lands or the risk is explicitly accepted.** (Note: the limited dashboard, /insights, /family/wishlists, and /family/pulse all already gate the `/members` fetch for limited; only /shopping-list consumes it.)
